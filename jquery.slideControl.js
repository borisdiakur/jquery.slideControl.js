...
(function ($, window, document) {
    ...
    function _getInfoObject($slideControl_, $slider_) {
        var sliderOffset = ($slider_.offset().left - $slideControl_.offset().left) + ($slider_.width() / 2),
            slideControlWidth = $slideControl_.width(), stepWidth = $slideControl_.data('steps') ? slideControlWidth / ($slideControl_.data('steps') - 1) : null,
            targetStep = $slideControl_.data('steps') ? Math.round(sliderOffset / stepWidth) : null,
            relativeTargetPosition = $slideControl_.data('steps') ? stepWidth * targetStep / slideControlWidth : sliderOffset / slideControlWidth, infoObject;
    ...
    function _slideToStep(slideToInfoObj_) {
        var $slideControl = slideToInfoObj_.slideControl,
    ...
    function _slideToRelativePosition(slideToInfoObj_) {
        var $slideControl = slideToInfoObj_.slideControl, $slider = $slideControl.find('.slider'), infoObj = _getInfoObject($slideControl, $slider);
    ...
    function _onMouseTouchMove(event_) {
        var $slideControl = event_.data.slideControl, $slider = event_.data.slider;
        ...
        if ($slider.hasClass('dragable')) {
            ...
            var pageX = (event_.originalEvent && event_.originalEvent.touches && event_.originalEvent.touches[0].pageX) ? event_.originalEvent.touches[0].pageX : event_.pageX,
    ...
    function _initialize(context_, settings_) {
        var $slideControl = $(context_), $slider = $slideControl.find('.slider'),
            slideControlWidth = $slideControl.width(), stepWidth = slideControlWidth / (settings_.steps - 1),
        ...
        $slider.css({
            'margin-left': $slider.width() / -2, 'top': ($slideControl.height() / 2) - ($slider.height() / 2)
        });
        ...
        if (settings_.steps && !isNaN(settings_.currentStep)) {
            ...
            if (!isNaN(settings_.currentRelativePosition)) {
        ...
        $slideControl.on('click', function (event_) {
            ...
            if ($slideControl.data('steps')) {
                var slideControlWidth = $slideControl.width(),
